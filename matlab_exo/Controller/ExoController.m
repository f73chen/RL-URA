classdef ExoController < handle
    ...
    methods
        function obj = ExoController(jointComponent, P, D, I, N ,dt, upT, gain, sat)
...
%             step(H,Hd)
        ...
        function f = update(obj,state,ref,t,outPut)
            
            p = obj.joint.getCoordinate().getValue(state);
            % error
            obj.e = ref - p;
%             if strcmp(obj.joint, 'ankle_r')
%                 obj.u = outPut(end,end-4);
%                 f = 0;
%             elseif strcmp(obj.joint, 'ankle_l')
%                 obj.u = outPut(end,end-1);
%                 f = 0;
%             else
                % current position:
